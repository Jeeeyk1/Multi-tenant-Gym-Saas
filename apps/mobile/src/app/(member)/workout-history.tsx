...
function typeColor(workoutType: string) {
  return TYPE_META[workoutType]?.color ?? '#6B7280';
}
...
export default function WorkoutHistoryScreen() {
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        headerTitle: { fontSize: 17, color: C.text, ...FONT.semibold },
        ...
        summaryLabel: { fontSize: 11, color: C.textMuted, ...FONT.regular, marginTop: 2 },
        ...
        sessionCalories: { fontSize: 11, color: C.textMuted, marginTop: 2 },
        ...
        emptyTitle: { fontSize: 20, color: C.text, ...FONT.bold },
        emptySub: { fontSize: 14, color: C.textSecondary, textAlign: 'center', marginBottom: SPACING.md },
        emptyBtn: {
          borderRadius: RADIUS.button,
          paddingVertical: 14,
  ...
  const sessionsQ = useWorkoutSessions();
  ...
  const isLoadingMore = sessionsQ.isFetchingNextPage;
  const refreshing = sessionsQ.isRefetching && !isLoadingMore;
  const hasMore = sessionsQ.hasNextPage;

  const onRefresh = () => sessionsQ.refetch();
  const loadMore = () => sessionsQ.fetchNextPage();
